const { checkgame } = require('./module');
...
wss.on('connection', function connection(ws) {
	...
	ws.on('message', function incoming(message) {
		...
		var update = found;
		if(found['map'][message]) return ws.terminate();
		update['map'][message] = ws.type;
		var cg = checkgame(found['map']);
		if(cg == "win" || cg == "tie") {
			ws.send('{"status":"' + (cg == "win" ? "winner" : "tie" ) + '", "row":'+ message + '}');
			ws.opponent.send('{"status":"' + (cg == "win" ? "loser" : "tie" ) + '", "row":'+ message + '}');
			games = games.filter(function(g) { return g.gameid != ws.gameid; });
	...
	ws.on('close', () => {
		if(ws.opponent) {
			...
			setTimeout(function () {
				...
				ws.opponent.send(JSON.stringify({'status':ws.opponent.status}));
			}, 800);
		}
		return ws.terminate();
	});
...
setInterval(function () {
	wss.clients.forEach(function each(ws) {
		...
		if(((new Date() - ws.time) / 1000) >= 15) {
			...
			var cg = checkgame(found['map']);
			if(cg == "win" || cg == "tie") {
				ws.send('{"status":"' + (cg == "win" ? "winner" : "tie" ) + '", "row":'+ selectedIndex + '}');
